...
- `taseo_marketing_should_print`, the marketing category's consent gate, covering Meta Pixel, Google Ads and Bing UET. Defaults to `true`, like the gates beside it. `taseo_meta_pixel_should_print` is unchanged and still honoured for the pixel specifically; new vendors get no per-vendor gate, because dropping one vendor for one request is what `taseo_tracking_tag_ids` is for.
