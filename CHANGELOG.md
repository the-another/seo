...
### Changed
- Titles & Templates tab: each row is now named for its registered post type, taxonomy, or system page (e.g. "Products", "Home page") instead of its raw `post:product`-style key, with that key still shown beneath in `<code>` for reference. Every title/meta description input has its own visible label bound to it by `id`, with multi-input rows grouped in a `<fieldset>` carrying a screen-reader legend that names the row. The tab is split into three sectioned tables — Post types, Taxonomies, System pages — each under its own heading, with a plain-language intro explaining what the title and meta description fields do. Validation errors now name the row in plain language too (e.g. "Products: %%price%% is not available for this content type…") instead of printing its internal `post:product` key.
